...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        return LineH(309); //309是对的 500是为了测试
    }
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        static NSString *cellID = @"BaseScrollHeaderCell";
        BaseScrollHeaderCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
        if (!cell) {
            cell = [[BaseScrollHeaderCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.backgroundColor = UICOLOR_RANDOM_COLOR();
        
        cell.navBigLabel.text = @"Find More";
        cell.titleLabel.text = @"发现";
        [cell.rightButton setTitle:@"我喜欢的" forState:(UIControlStateNormal)];
        [cell.rightButton setImage:UIIMAGE_FROM_NAME(@"爱心") forState:(UIControlStateNormal)];
