...
- (void)baseInit {
    _notSelectedStar = [UIImage imageNamed:@"Evaluation_after_class_not"];
    _selectedStar = [UIImage imageNamed:@"Evaluation_after_class_have"];
...
- (void)layoutSubviews {
    ...
    float imageWidth = LineW(self.starWidth);
    
    for (int i = 0; i < _starViews.count; ++i) {
        UIImageView *imageView = [_starViews objectAtIndex:i];
        CGRect imageFrame = CGRectMake(_leftMargin + i*(_midMargin+imageWidth), 0, imageWidth, imageWidth);
