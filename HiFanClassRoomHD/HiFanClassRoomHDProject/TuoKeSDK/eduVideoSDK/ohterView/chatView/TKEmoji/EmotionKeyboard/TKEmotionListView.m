...
@interface TKEmotionListView() <UIScrollViewDelegate>
/** 显示所有表情的UIScrollView */
...
@implementation TKEmotionListView
...
//        pageControl.backgroundColor = [UIColor blueColor];
        [pageControl setValue:[UIImage imageNamed:@"TKEmoji.bundle/compose_keyboard_dot_selected"] forKeyPath:@"_currentPageImage"];
        [pageControl setValue:[UIImage imageNamed:@"TKEmoji.bundle/compose_keyboard_dot_normal"] forKeyPath:@"_pageImage"];
...
- (void)setEmotions:(NSArray *)emotions
{
    ...
    // 设置总页数
    int totalPages = (int)(emotions.count + TKEmotionMaxCountPerPage - 1) / TKEmotionMaxCountPerPage;
    ...
    for (int i = 0; i<totalPages; i++) {
        ...
        TKEmotionGridView *gridView = nil;
        if (i >= currentGridViewCount) { // 说明HMEmotionGridView的个数不够
            gridView = [[TKEmotionGridView alloc] init];
//            gridView.backgroundColor = HMRandomColor;
        ...
        int loc = i * TKEmotionMaxCountPerPage;
        int len = TKEmotionMaxCountPerPage;
    ...
    for (int i = totalPages; i<currentGridViewCount; i++) {
        TKEmotionGridView *gridView = self.scrollView.subviews[i];
        gridView.hidden = YES;
    }
